...
(() => {
  // utils.js
  var uindow = unsafeWindow;
  var log = (f, ...s) => uindow.console.log(`%c[exlg] ${f}`, "color: #0e90d2;", ...s);
  var warn = (f, ...s) => uindow.console.warn(`%c[exlg] ${f}`, "color: #0e90d2;", ...s);
  var error = (f, ...s) => {
  ...
  Date.prototype.format = function(f, UTC) {
    ...
    for (const r in re)
      if (RegExp(`(${r})`).test(f))
        f = f.replace(RegExp.$1, ("000" + re[r]).substr(re[r].toString().length + 3 - RegExp.$1.length));
    return f;
  };
  ...
  var version_cmp = (v1, v2) => {
    ...
    if (n1 === n2)
      return op(...[e1, e2].map((e) => e ? exs.findIndex((ex) => ex === e) : Infinity));
    const [m1, m2] = [n1, n2].map((n) => n.split("."));
    for (const [k2, m] of m1.entries())
      if (m !== m2[k2])
        return op(+m || 0, +m2[k2] || 0);
  };
  var cur_time = (ratio = 1) => {
    let d = new Date();
    return ~~(d.getTime() / ratio);
  };
  var lg_content = (url) => new Promise((res, rej) => $.get(url + (url.includes("?") ? "&" : "?") + "_contentOnly=1", (data) => {
    if (data.code !== 200)
      rej(`Requesting failure code: ${res.code}.`);
    res(data);
  }));
  var lg_alert = uindow.show_alert ? (msg2, title = "exlg 提醒您") => uindow.show_alert(title, msg2) : (msg2, title = "exlg 提醒您") => uindow.alert(title + "\n" + msg2);
  var csrf_token = null;
  var lg_post = (url, data) => $.ajax({
    url,
    data,
    headers: {
      "x-csrf-token": csrf_token === null ? csrf_token = $("meta[name=csrf-token]").attr("content") : csrf_token,
      "content-type": "application/json"
    },
    method: "post"
  });
  var cs_get = ({ url, onload, onerror = (err) => error(err) }) => GM_xmlhttpRequest({
    url,
    method: "GET",
    onload,
    onerror
  });
  var springboard = (param, styl) => {
  ...
  var exlg_dialog_board = {
    ...
    show_exlg_alert(text = "", title = "exlg 提醒您", onaccepted = () => {
    }, autoquit = true) {
  ...
  var exlg_alert = (...arg) => exlg_dialog_board.show_exlg_alert(...arg);
  var register_badge = async () => {
  ...
  var mod = {
    ...
    path_dash_board: [
      ...
      "@debug/exlg-setting/((index|bundle).html)?"
    ],
    ...
    reg_main: (name, info, path, data, func, styl) => mod.reg("@" + name, info, path, data, (arg) => (func(arg), false), styl),
    reg_user_tab: (name, info, tab, vars, data, func, styl) => mod.reg(name, info, "@/user/.*", data, (arg) => {
      const $tabs = $(".items");
      const work = () => {
    ...
    reg_chore: (name, info, period, path, data, func, styl) => {
      if (typeof period === "string") {
        ...
        if (!isNaN(num) && unit)
          ...
          error(`Parsing period failed: "${period}"`);
      }
      name = "^" + name;
      data = {
        ...data,
      ...
      mod.reg(name, info, path, data, (arg) => {
        ...
        if (arg.named || !last || now - last > period) {
          ...
          log(`Pending chore: "${name}"`);
      });
    },
    reg_board: (name, info, data, func, styl) => mod.reg(name, info, "@/", data, (arg) => {
      const icon_b = `
      ...
      if (!$board.length)
        $board = $(`
                ...
                `).prependTo(".lg-right.am-u-md-4"), $board[0].firstChild.style["font-size"] = "1em";
      func({ ...arg, $board: $(`<div></div>`).appendTo($board) });
    }, styl),
    reg_hook: (name, info, path, data, func, hook, styl) => mod.reg(name, info, path, data, (arg) => {
      func(arg);
      $("body").bind("DOMNodeInserted", (e) => hook(e) && func(arg));
    }, styl),
    reg_hook_new: (name, info, path, data, func, hook, darg, styl) => mod.reg(name, info, path, data, (arg) => {
    ...
    preload: (name) => {
      ...
      const exe = (m, named) => {
        if (!m)
          error(`Preloading named mod but not found: "${name}"`);
      ...
      for (const [name2, m] of mod._.entries()) {
        if (sto[name2].on && m.path.some((re) => new RegExp(re, "g").test(pn))) {
          m.willrun = true;
          if ("pre" in m)
            mod._.set(name2, exe({ name: name2, ...m }));
        }
      }
    },
    execute: (name) => {
      const exe = (m, named) => {
        if (!m)
          error(`Executing named mod but not found: "${name}"`);
        if (m.styl)
          GM_addStyle(m.styl);
  ...
  mod.reg("benben", "全网犇犇", "@/", {
      ...
      if (uindow.feedMode === "all-exlg") {
        cs_get({
          ...
          onload: (res) => {
                    ...
                    <div class="am-comment-main">
                        <header class="am-comment-hd">
                            <div class="am-comment-meta">
                                ...
                                ${new Date(m.time * 1e3).format("yyyy-mm-dd HH:MM")}
            ...
            `).appendTo($("ul#feed")).find("a[name=feed-reply]").on("click", () => {
              scrollToId("feed-content");
              setTimeout(() => $("textarea").trigger("focus").val(` || @${m.user.name} : ${m.content}`).trigger("input"), 50);
            }));
          }
        });
        if (msto.source === "shy") {
    ...
    $(`<li class="feed-selector" id="exlg-benben-selector" data-mode="all-exlg" exlg="exlg"><a style="cursor: pointer">全网动态</a></li>`).appendTo($sel.parent()).on("click", (e) => {
  ...
  mod.reg_board("benben-ranklist", "犇犇龙王排行榜", {
    ...
    $board.find("#refresh-bbrnk").on("click", () => {
      $list.html(""), refresh();
    });
    refresh();
  }, `
...
`);
  ...
  mod.reg_hook_new("code-block-ex", "代码块优化", "@/.*", {
    ...
    const get_lang = ($code) => {
      let lang = "undefined";
      if (isRecord)
        return $($(".value.lfe-caption")[0]).text();
      if ($code.attr("data-rendered-lang"))
        lang = $code.attr("data-rendered-lang");
    ...
    args.each((_, e, $pre = $(e)) => {
      if (e.parentNode.className === "mp-preview-content" || e.parentNode.parentNode.className === "mp-preview-area")
        return;
      const $btn = isRecord ? $pre.children(".copy-btn") : $(`<div class="exlg-copy">复制</div>`).on("click", () => {
        if ($btn.text() !== "复制")
          return;
      ...
      $code.css("font-family", msto.copy_code_font || void 0);
      if (!$code.hasClass("hljs"))
        $code.addClass("hljs").css("background", "white");
...
`);
  ...
  mod.reg_main("dash-board", "控制面板", mod.path_dash_board, {
    ...
    const modules = [["Modules", "功能", "tunes", false], ["Core", "核心", "bug_report", true]].map(([name, description, icon, is_main]) => ({
      name,
      description,
      icon,
      children: (() => {
        ...
        mod._.forEach((m, nm) => {
          if (nm.startsWith("@") === is_main)
            arr.push({
              ...
              name: nm.replace(/^[@^]/g, ""),
              description: m.info,
              settings: Object.entries(mod.data[nm].lvs).filter(([k, s]) => k !== "on" && !s.priv).map(([k, s]) => ({
                name: k,
  ...
  mod.reg_hook_new("dash-bridge", "控制桥", "@/.*", {
    ...
    if (["exlg", "gh_index", "debug"].indexOf(msto.source) === -1)
      msto.source = "exlg";
    ...
    const $btn = $(`<div id="exlg-dash" exlg="exlg">exlg</div>`).prependTo(args).css("backgroundColor", {
      exlg: "cornflowerblue",
      gh_index: "darkblue",
      debug: "steelblue"
    }[msto.source]).css("margin-top", args.hasClass("nav-container") ? "5px" : "0px");
    ...
    if (msto.enable_rclick)
      $btn.bind("contextmenu", () => false).on("mousedown", (e) => {
        if (!e.button)
          _jump_settings();
        else if (e.button === 2) {
          msto.source = {
    ...
    if (create_window) {
      ...
      _list.forEach((e) => {
        const $div = $(`<div id="${e.tag}-div"><span class="exlg-windiv-left-tag">${e.title}</span></div>`).appendTo($bdiv), $span = $("<span></span>").appendTo($div);
        ...
        if (e.title === "vers") {
          ...
          const $check_btn = $(`<button class="exlg-windiv-btn" style="background-color: red;border-color: red;float: right;margin: 0 10px 0 0;">刷新</button>`), $operator = $span.find("#vers-comp-operator"), $latest = $span.find("#latest-version"), $fuckingdots = $span.find("#annoyingthings");
          const _check = () => {
            $("#exlg-update").remove();
            springboard({ type: "update" }).appendTo($("body")).hide();
            uindow.addEventListener("message", (e2) => {
              if (e2.data[0] !== "update")
                return;
              e2.data.shift();
              const latest = e2.data[0], version = GM_info.script.version, op = version_cmp(version, latest);
              ...
              if (op === "==")
                msto.latest_ignore = GM_info.script.version;
              if (uindow.novogui)
    ...
    if ($tmp.length)
      return { result: $tmp.length, args: $tmp[0].tagName === "DIV" ? $($tmp[0].firstChild) : $tmp };
    else
      return { result: 0 };
...
`);
  ...
  mod.reg("exlg-dialog-board", "exlg_公告板", "@/.*", {
    ...
    const wrapper = $wrapper[0], container = wrapper.firstElementChild, header = container.firstElementChild.firstElementChild.firstElementChild, content = container.lastElementChild.firstElementChild, close_btn = container.firstElementChild.lastElementChild;
...
`);
  ...
  mod.reg("emoticon", "表情输入", ["@/paste", "@/discuss/.*", "@/"], {
    ...
    const emo = [
      ...
      if (typeof e === "string")
        return {
          type: emt.EMO,
          name: [e],
          slug: i >= 10 ? String.fromCharCode(97 + (i - 10)) : String.fromCharCode(48 + i)
        };
      return { type: emt.TXT, ...e };
    });
    const emo_url = msto.src === "github" ? ({ slug }) => `//cdn.jsdelivr.net/gh/extend-luogu/extend-luogu/img/emoji/${slug}` : msto.src === "啧.tk" ? ({ name }) => `//${msto.src}/${name[0]}` : ({ slug }) => `//${msto.src}/${slug}`;
    if (msto.benben && location.pathname === "/") {
      const $txt2 = $("#feed-content"), txt = $txt2[0];
      $("#feed-content").before("<div id='emo-lst'></div>");
      emo.forEach((m) => {
        const $emo = $(m.type === emt.EMO ? `<button class="exlg-emo-btn" exlg="exlg"><img src="${emo_url(m)}" /></button>` : `<button class="exlg-emo-btn" exlg="exlg">${m.name_display}</button>`).on("click", () => {
        ...
        if (m.width)
          $emo.css("width", m.width + "px");
        else if (m.type === emt.EMO)
          $emo.css("width", "40px");
        else
          $emo.css("width", "83px");
      });
    ...
    if (msto.show)
      $emo_menu.addClass("exlg-show-emo"), $ground.addClass("exlg-show-emo");
    $show_hide.on("click", () => {
    ...
    if (msto.height_limit)
      $emo_menu.addClass("exlg-show-emo-short"), $ground.addClass("exlg-show-emo-short");
    else
      $emo_menu.addClass("exlg-show-emo-long"), $ground.addClass("exlg-show-emo-long");
    $set_height.on("click", () => {
    ...
    emo.forEach((m) => {
      const $emo = $(m.type === emt.EMO ? `<button class="exlg-emo-btn" exlg="exlg"><img src="${emo_url(m)}" /></button>` : `<button class="exlg-emo-btn" exlg="exlg">${m.name_display}</button>`).on("click", () => $txt.trigger("focus").val(`![](${emo_url(m)})`).trigger("input")).appendTo($emo_menu);
      if (m.width)
        $emo.css("width", m.width + "px");
      else if (m.type === emt.EMO)
        $emo.css("width", "40px");
...
`);
  ...
  mod.reg("keyboard-and-cli", "键盘操作与命令行", "@/.*", {
    ...
    const cmds = [
      gcmd("help", "[cmd: string]", [
        ...
        if (!cmd)
          ...
          const f = cmds[cmd];
          if (!f)
            return cli_error`help: unknown command "${cmd}"`;
      ...
      gcmd("cdd", "forum: string", [
        ...
        if (!tar)
          return cli_error`cdd: unknown forum "${forum}"`;
      ...
      gcmd("cc", "[name: char]", [
        ...
        if (tar)
          ...
          cli_error`cc: unknown target "${name}"`;
      }),
      gcmd("mod", "action: string, [name: string]", [
        ...
        switch (action) {
          ...
          case "toggle":
            if (!mod.has(name))
              return cli_error`mod: unknown mod "${name}"`;
            sto[name].on = {
              enable: () => true,
              disable: () => false,
              toggle: (now) => !now
    ...
    const parse = (cmd) => {
      ...
      const f = cmds[n];
      if (!f)
        return cli_error`exlg: unknown command "${n}"`;
      let i = -1, a;
      for ([i, a] of tk.entries()) {
        const t = f.arg[i].type;
        if (t === "number" || t === "integer")
          tk[i] = +a;
        if (t === "char" && a.length === 1 || t === "number" && !isNaN(tk[i]) || t === "integer" && !isNaN(tk[i]) && !(tk[i] % 1) || t === "string")
          ;
        else
          return cli_error`${n}: illegal param "${a}", expected type ${t}.`;
      }
      if (f.arg[i + 1]?.essential)
        return cli_error`${n}: lost essential param "${f.arg[i + 1].name}"`;
      f.fn(...tk);
    };
...
`);
  ...
  mod.reg("dbc-jump", "双击题号跳题", "@/.*", null, () => {
    $(document).on("dblclick", (e) => {
      ...
      const url = e.ctrlkey ? $(".ops > a[href*=blog]").attr("href") + "solution-" : "https://www.luogu.com.cn/problem/";
  ...
  mod.reg_hook_new("back-to-contest", "返回比赛题单", [
    ...
    const $info_rows = args.$info_rows, $pre = $(`<a class="exlg-back-to-contest"></a>`), cid = args.cid, pid = args.pid;
    if (!pid || !cid)
      return;
    $pre.attr("href", `/contest/${cid}#problems`).html(`<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="door-open" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" class="svg-inline--fa fa-door-open fa-w-20">
            ...
            </svg>返回列表`).appendTo($info_rows);
  }, (e) => {
    const tar = e.target, cid = lg_dat.contest.id, pid = lg_dat.problem.pid;
...
`);
  mod.reg_hook_new("submission-color", "记录难度可视化", "@/record/list.*", null, async ({ args }) => {
    if (args && args.type === "show") {
      if ($("div.problem > div > a > span.pid").length && !$(".exlg-difficulty-color").length) {
        const u2 = await lg_content(location.href);
        const dif2 = u2.currentData.records.result.map((u3) => u3.problem.difficulty);
        $("div.problem > div > a > span.pid").each((i, e, $e = $(e)) => {
          $e.addClass("exlg-difficulty-color").addClass(`color-${dif2[i]}`);
        });
      }
      return;
    }
    if ($(".exlg-difficulty-color").length)
      return;
    const u = await lg_content(location.href);
    const dif = u.currentData.records.result.map((u2) => u2.problem.difficulty);
    ...
    const tar = e.target;
    if (!tar || !tar.tagName)
      return { args: msg.COMMENT_TAG, result: false };
    if (tar.tagName.toLowerCase() === "a" && (tar.href || "").includes("/problem/") && ` ${tar.parentNode.parentNode.className} `.includes(" problem ")) {
      if (!tar.parentNode.parentNode.parentNode.nextSibling)
        return { args: { type: "modified - update", target: tar.parentNode.parentNode.parentNode.parentNode }, result: true };
      else
        return { args: { type: "modified - not the last one.", target: null }, result: false };
    }
  ...
  // modules/original-difficulty.js
      ...
      if (location.pathname.includes("CF")) {
        ...
        cs_get({
          ...
          onload: (res) => {
            ...
            resolve(rv ? rv.substring(1) : void 0);
          },
          onerror: (err) => {
            error(err);
            reject(err);
        ...
        if (!(pid in dif))
          ...
          resolve(Math.round(dif[pid] >= 400 ? dif[pid] : 400 / Math.exp(1 - dif[pid] / 400)));
      }
    });
  }, ({ pred }) => {
  ...
  mod.reg("rand-problem-ex", "随机跳题_ex", "@/", {
    ...
    const func_jump_problem = (str) => {
      if (judge_problem(str))
        str = str.toUpperCase();
      if (str === "" || typeof str === "undefined")
        uindow.show_alert("提示", "请输入题号");
      else
        location.href = "https://www.luogu.com.cn/problemnew/show/" + str;
    };
    ...
    <br>
    <ul></ul>
    </span>`).appendTo($btn_list).hide().on("mouseenter", () => {
      mouse_on_board = true;
    }).on("mouseleave", () => {
    ...
    $("#exlg-dash-0").on("mouseenter", () => {
      ...
      }, [$exrand_diff, msto.exrand_difficulty], [$exrand_srce, msto.exrand_source]);
      $board.show();
    }).on("mouseleave", () => {
      mouse_on_dash = false;
      if (!mouse_on_board) {
    ...
    const exrand_poi = async () => {
      ...
      const list = res.currentData.problems.result, rand_idx = Math.floor(Math.random() * list.length), pid = list[rand_idx].pid;
...
`);
  ...
  // modules/rand-training-problem.js
  mod.reg_hook_new("rand-training-problem", "题单内随机跳题", "@/training/[0-9]+(#.*)?", {
    ...
    if (!args.length)
      return;
    $(args[0].firstChild).clone(true).appendTo(args).text("随机跳题").addClass("exlg-rand-training-problem-btn").on("click", () => {
      ...
      const pid = ~~(Math.random() * 1e6) % candProbList.length;
...
`);
  ...
  mod.reg_hook_new("sponsor-tag", "标签显示", ["@/", "@/paste", "@/discuss/.*", "@/problem/.*", "@/ranking.*"], {
    ...
    const add_badge = ($e) => {
      if (!$e || $e.hasClass("exlg-badge-username"))
        return;
      if (!/\/user\/[1-9][0-9]{0,}/.test($e.attr("href")))
        return;
      ...
      const $badge = $(user_uid === "100250" ? `<span class="am-badge am-radius lg-bg-red" style="margin-left: 4px;">${tag}</span>` : `<span class="exlg-badge">${tag}</span>`).off("contextmenu").on("contextmenu", () => false).on("mousedown", (e) => {
        if (e.button === 2)
          location.href = "https://www.luogu.com.cn/paste/asz40850";
        else if (e.button === 0)
          register_badge();
      });
      let $tar = $e;
...
`);
  ...
  mod.reg("tasklist-ex", "任务计划_ex", "@/", {
    ...
    $.each($("div.tasklist-item"), (_, prob, $e = $(prob)) => {
      ...
      if ($e.find("i").hasClass("am-icon-check"))
        $e.addClass("tasklist-ac-problem");
    });
    ...
    if (msto.rand_problem_in_tasklist) {
      ...
      $btn.addClass("exlg-rand-tasklist-problem-btn").click(() => {
        let tid = ~~(Math.random() * 1e6) % actTList.length;
  ...
  mod.reg_chore("update", "检查更新", "1D", mod.path_dash_board, null, () => {
    ...
    uindow.addEventListener("message", (e) => {
      if (e.data[0] !== "update")
        return;
      e.data.shift();
      const latest = e.data[0], version = GM_info.script.version, op = version_cmp(version, latest);
      const l = `Comparing version: ${version} ${op} ${latest}`;
      log(l);
    });
  });
    ...
    $(".introduction > *").each((_, e, $e = $(e)) => {
      ...
      switch (ins) {
        case "html":
          ...
          break;
        case "frame":
          $e.replaceWith(springboard({ type: "page", url: encodeURI(arg[0]), confirm: true }, `width: ${arg[1]}; height: ${arg[2]};`));
          break;
        case "blog":
          if ($blog.text().trim() !== "个人博客")
            return;
          $blog.attr("href", arg);
          $e.remove();
          break;
...
`);
  ...
  // modules/user-problem-color.js
  var brds = {
    SUBMITTED_PROBLEMS: 0,
    PASSED_PROBLEMS: 1
  };
  var last_ptr = -1;
  var last_board = brds.SUBMITTED_PROBLEMS;
  var cosflag = -1;
  var ta = null;
  var my = null;
  mod.reg_hook_new("user-problem-color", "题目颜色数量和比较", "@/user/[0-9]{0,}.*", {
    ...
    const func = async ($prb, _flag) => {
      if (ta === null) {
        ...
        ta = lg_dat.passedProblems, my = /* @__PURE__ */ new Set();
      ...
      if (_flag) {
        ...
        $ps.querySelectorAll("a").forEach((p, d) => {
          if (d < ta.length && my.has(ta[d].pid)) {
            same++;
            p.style.backgroundColor = "rgba(82, 196, 26, 0.3)";
          }
        });
      }
      $("#exlg-problem-count-1").html(`<span class="exlg-counter" exlg="exlg">${ta.length} <> ${my.size} : ${same}<i class="exlg-icon exlg-info" name="ta 的 &lt;&gt; 我的 : 相同"></i></span>`);
    };
    const _color = (id) => `rgb(${color[id][0]}, ${color[id][1]}, ${color[id][2]})`;
    if (typeof args === "object" && args.message === msg.ADD_COMPARE) {
      if (!msto.problem_compare || lg_dat.user.uid === lg_usr.uid)
        return;
      func([114514, 1919810], false);
      return;
    }
    args.forEach((arg) => {
      if (arg.target.href === "javascript:void 0")
        return;
      arg.target.style.color = _color([(arg.board_id ? lg_dat.passedProblems : lg_dat.submittedProblems)[arg.position].difficulty]);
      if (arg.board_id === brds.PASSED_PROBLEMS && arg.position === lg_dat.passedProblems.length - 1 || lg_dat.passedProblems.length === 0 && arg.board_id === brds.SUBMITTED_PROBLEMS && arg.position === lg_dat.submittedProblems.length - 1) {
        $(".exlg-counter").remove();
        ...
        for (let i = 0; i < 2; ++i) {
          const $ps = $prb[i];
          const my2 = lg_dat[["submittedProblems", "passedProblems"][i]];
          $ps.before($(`<span id="exlg-problem-count-${i}" class="exlg-counter" exlg="exlg">${my2.length}</span>`)[0]);
        }
        if (!msto.problem_compare || lg_dat.user.uid === lg_usr.uid)
          return;
        func($prb, true);
    ...
    if (location.hash !== "#practice")
      return { result: false, args: { message: msg.NOT_AT_PRACTICE_PAGE } };
    if (!lg_dat.submittedProblems.length && !lg_dat.passedProblems.length) {
      if (e.target.className === "card padding-default") {
        if ($(e.target).children(".problems").length) {
          const my2 = lg_dat[["submittedProblems", "passedProblems"][cosflag]];
          $(e.target.firstChild).after(`<span id="exlg-problem-count-${cosflag}" class="exlg-counter" exlg="exlg" style="margin-left: 5px">${my2.length}</span>`);
          if (++cosflag > 1)
            return { result: true, args: { message: msg.ADD_COMPARE } };
    ...
    if (!e.target.tagName)
      return { result: false, args: { message: msg.COMMENT_TAG } };
    ...
    const gpid = (o) => o ? o.pid : void 0;
    const tar = e.target, _onc = [gpid(lg_dat.submittedProblems[0]), gpid(lg_dat.passedProblems[0])].indexOf(tar.href.slice(33)), _onchange = !(_onc === -1);
    return {
      result: true,
      args: [{
        onchange: _onchange,
    ...
    if (!lg_dat.submittedProblems.length && !lg_dat.passedProblems.length) {
      ...
      for (let i = 0; i < 2; ++i) {
        const $ps = $($prb[i]);
        const my2 = lg_dat[["submittedProblems", "passedProblems"][i]];
        $ps.before(`<span id="exlg-problem-count-${i}" class="exlg-counter" exlg="exlg">${my2.length}</span>`);
      }
...
`);
  ...
  // modules/virtual-participation.js
  mod.reg("virtual-participation", "创建重现赛", "@/contest/[0-9]*(#.*)?", {
    ...
    if (lg_dat.contest.id.toString() === msto.vp_id) {
      ...
      if (lg_dat.contestProblems.length === 0) {
        ...
        }, lg_dat.contest.startTime * 1e3 - cur_time() - 500);
      }
      return;
    }
    if (lg_dat.contest.endTime > cur_time(1e3)) {
      warn("Contest has not started or ended.");
      return;
    }
    $("<button id='exlg-vp' class='lfe-form-sz-middle'>重现比赛</button>").appendTo($("div.operation")).click(async () => {
      exlg_alert(`<div>
        ...
        let st = new Date(pa[0], pa[1] - 1, pa[2], pb[0], pb[1], 0, 0);
        st = st.getTime() / 1e3;
        msto.orig_dat.pids = msto.orig_dat.scrs = "";
        $.each(lg_dat.contestProblems, (id, vl) => {
          if (id)
            msto.orig_dat.pids += ",", msto.orig_dat.scrs += ",";
          msto.orig_dat.pids += '"' + vl.problem.pid + '"', msto.orig_dat.scrs += `"${vl.problem.pid}": ${vl.problem.fullScore}`;
        });
