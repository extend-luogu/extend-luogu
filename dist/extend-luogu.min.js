...
`+e),X=null,B=(e,t)=>o.ajax({url:e,data:t,headers:{"x-csrf-token":X===null?X=o("meta[name=csrf-token]").attr("content"):X,"content-type":"application/json"},method:"post"}),P=({url:e,onload:t,onerror:r=n=>T(n)})=>GM_xmlhttpRequest({url:e,method:"GET",onload:t,onerror:r}),I=(e,t)=>{let r=new URLSearchParams;for(let l in e)r.set(l,e[l]);let n=o(`
    ...
    `);let t=()=>{r.prop("disabled",!0),o.get("/api/user/search?keyword="+o("[name=username]").val(),n=>{n.users[0]?location.href="/user/"+n.users[0].uid:(r.prop("disabled",!1),G("无法找到指定用户"))})},r=o("#search-user").on("click",t);o("#search-user-input").keydown(n=>{n.key==="Enter"&&t()})});c.reg_board("benben-ranklist","犇犇龙王排行榜",{show:{ty:"boolean",dft:!0,info:["Show in default","是否默认展开"]}},({msto:e,$board:t})=>{t.html(`<h3 id="bb-rnklst-h2">犇犇排行榜 <span id="bb-rnklst-btn" class="bb-rnklst-span"> [<a>${e.show?"收起":"展开"}</a>]</span><span style="float: right;" class="bb-rnklst-span"> [<a id="refresh-bbrnk">刷新</a>]</span></h3><div style="display: ${e.show?"block":"none"}" id="bb-rnklst-div"></div>`);let r=t.find("#bb-rnklst-div"),n=t.find("#bb-rnklst-btn > a").on("click",()=>{e.show=!e.show,n.text(e.show?"收起":"展开"),r.toggle()}),l=P({url:"https://bens.rotriw.com/ranklist?_contentOnly=1",onload:function(i){o(JSON.parse(i.response)).each((a,s)=>{o(`<div class="bb-rnklst-${a+1}">
...
*M virtual-participation
 : 修复了无法跳转的 bug
 : 优化了体验
`.trim();c.reg_main("version-data","版本数据","@tcs2/release/exlg-nextgen",null,()=>h.parent.postMessage([document.body.innerText],"*"));c.reg_chore("update","检查更新","1D",c.path_dash_board,null,()=>{o("#exlg-update").remove(),I({type:"update"}).appendTo(o("body")).hide(),h.addEventListener("message",e=>{if(e.data[0]!=="update")return;e.data.shift();let t=e.data[0],r=GM_info.script.version,n=R(r,t),l=`Comparing version: ${r} ${n} ${t}`;$(l)})});c.reg("update-log","更新日志显示","@/.*",{last_version:{ty:"string",priv:!0}},({msto:e})=>{if(location.href.includes("blog"))return;let t=GM_info.script.version,r=n=>{let l=`<div class="exlg-update-log-text" style="font-family: ${L["code-block-ex"].copy_code_font};">`;return n.split(`
