...
`);c.reg_hook_new("submission-color","记录难度可视化","@/record/list.*",null,async({args:e})=>{if(e&&e.type==="show"){if(o("div.problem > div > a > span.pid").length&&!o(".exlg-difficulty-color").length){let i=(await A(location.href)).currentData.records.result.map(l=>l.problem.difficulty);o("div.problem > div > a > span.pid").each((l,a,s=o(a))=>{s.addClass("exlg-difficulty-color").addClass(`color-${i[l]}`)})}return}if(o(".exlg-difficulty-color").length)return;let n=(await A(location.href)).currentData.records.result.map(r=>r.problem.difficulty);o(e.target).find("div.problem > div > a > span.pid").each((r,i,l=o(i))=>{l.addClass("exlg-difficulty-color").addClass(`color-${n[r]}`)})},e=>{let t=e.target;return!t||!t.tagName?{args:O.COMMENT_TAG,result:!1}:t.tagName.toLowerCase()==="a"&&(t.href||"").includes("/problem/")&&` ${t.parentNode.parentNode.className} `.includes(" problem ")?t.parentNode.parentNode.parentNode.nextSibling?{args:{type:"modified - not the last one.",target:null},result:!1}:{args:{type:"modified - update",target:t.parentNode.parentNode.parentNode.parentNode},result:!0}:{args:{type:"modified - not that one.",target:null},result:!1}},()=>({type:"show"}),"");c.reg("mainpage-discuss-limit","主页讨论个数限制",["@/"],{max_discuss:{ty:"number",dft:12,min:4,max:16,step:1,info:["Max Discussions On Show","主页讨论显示上限"],strict:!0}},({msto:e})=>{let t;location.href.includes("blog")||(o(".lg-article").each((n,r,i=o(r))=>{let l=r.childNodes[1];l&&l.tagName.toLowerCase()==="h2"&&l.innerText.includes("讨论")&&(t=i.children(".am-panel"))}),t.each((n,r,i=o(r))=>{n>=e.max_discuss&&i.hide()}))});c.reg("user-css","自定义样式表",".*",{css:{ty:"string"}},({msto:e})=>GM_addStyle(e.css));c.reg("benben-quickpost","CtrlEnter_发送犇犇","@/",null,()=>o("textarea").whenKey("CtrlEnter",()=>o("#feed-submit").click()));c.reg_chore("atdiff-fetch","获取_AtCoder_难度","10D","@/problem/AT.*",{atdiff:{ty:"string",priv:!0}},({msto:e})=>{let t={};S({url:"https://kenkoooo.com/atcoder/resources/problem-models.json",onload:n=>{let r=JSON.parse(n.responseText);for(let i in r)t[i]=r[i].difficulty;e.atdiff=JSON.stringify(t)}})});c.reg_pre("original-difficulty","显示原始难度",["@/problem/CF.*","@/problem/AT.*"],{cf_src:{ty:"enum",dft:"codeforces.com",vals:["codeforces.com","codeforces.ml"],info:["Codeforces problem source","CF 题目源"]}},async({msto:e})=>new Promise((t,n)=>{let r=location.pathname.match(/(CF|AT)([0-9]|[A-Z])*$/g)[0].substring(2);if(location.pathname.includes("CF")){let i=r.match(/^[0-9]*/g)[0],l=r.substring(i.length);S({url:`https://${e.cf_src}/problemset/problem/${i}/${l}`,onload:a=>{let s=o(a.responseText).find("span[title=Difficulty]").text().trim();t(s?s.substring(1):void 0)},onerror:a=>{M(a),n(a)}})}else{let i=JSON.parse(C["^atdiff-fetch"].atdiff),l=h.problem.description.match(RegExp("^.{22}[-./A-Za-z0-9_]*"))[0].match(RegExp("[^/]*$"));l in i?t(Math.round(i[l]>=400?i[l]:400/Math.exp(1-i[l]/400))):t(void 0)}}),({pred:e})=>{let t=document.querySelectorAll("div.field"),n=t[3].cloneNode(!0);t[3].after(n);let r=n.querySelectorAll("span");r[0].innerText="原始难度",r[1].innerText="获取中",e.then(i=>{i===void 0&&(i="不可用"),r[1].innerText=i})});c.reg("rand-problem-ex","随机跳题_ex","@/",{exrand_difficulty:{ty:"tuple",lvs:[{ty:"boolean",dft:!1,strict:!0,repeat:8}],priv:!0},exrand_source:{ty:"tuple",lvs:[{ty:"boolean",dft:!1,strict:!0,repeat:5}],priv:!0}},({msto:e})=>{let t=[["入门","red"],["普及-","orange"],["普及/提高-","yellow"],["普及+/提高","green"],["提高+/省选-","blue"],["省选/NOI-","purple"],["NOI/NOI+/CTSC","black"],["暂无评定","gray"]].map((m,w,b)=>({text:m[0],color:m[1],id:(w+1)%b.length})),n=[{text:"洛谷题库",color:"red",id:"P"},{text:"Codeforces",color:"orange",id:"CF"},{text:"SPOJ",color:"yellow",id:"SP"},{text:"AtCoder",color:"green",id:"AT"},{text:"UVA",color:"blue",id:"UVA"}],r=m=>{W(m)&&(m=m.toUpperCase()),m===""||typeof m>"u"?x.show_alert("提示","请输入题号"):location.href="https://www.luogu.com.cn/problemnew/show/"+m},i=!1,l=!1,a=o("input[name='toproblem']");a.after(a.clone()).remove(),a=o("input[name='toproblem']");let s=o(".am-btn[name='goto']");s.after(s.clone()).remove(),s=o(".am-btn[name='goto']");let d=s.parent();o(".am-btn[name='gotorandom']").text("随机");let g=o('<button class="am-btn am-btn-success am-btn-sm" name="gotorandomex">随机ex</button>').appendTo(d);s.on("click",()=>{/^[0-9]+.?[0-9]*$/.test(a.val())&&a.val("P"+a.val()),r(a.val())}),a.on("keydown",m=>{m.keyCode===13&&s.click()});let p=o(`<span id="exlg-exrand-window" class="exlg-window" style="display: block;">
    ...
    <ul></ul>
        ...
        <br>
        </span>`).appendTo(m),["exrand-enabled","已选择"],["exrand-disabled","未选择"]);w.forEach((E,z)=>{let j=o.double(P=>o(`<div class="exlg-smallbtn exlg-unselectable">${E.text}</div>`).css("background-color",`var(--lg-${E.color}-problem)`).appendTo(P),v[0],v[1]);o.double(P=>{j[P].on("click",()=>{j[P].hide(),j[1-P].show(),b[z]=!!P}),b[z]===!!P&&j[P].hide()},0,1)})},[T,t,e.exrand_difficulty],[L,n,e.exrand_source]),o("#exlg-dash-0").on("mouseenter",()=>{l=!0,o.double(([m,w])=>{o.double(([b,v])=>{m.children(b).children(".exlg-smallbtn").each((E,z,j=o(z))=>w[E]===v?j.show():j.hide())},[".exrand-enabled",!0],[".exrand-disabled",!1])},[T,e.exrand_difficulty],[L,e.exrand_source]),p.show()}).on("mouseleave",()=>{l=!1,i||setTimeout(()=>{i||p.hide()},200)});let f=async()=>{let m=o.double(([ce,ge,pe])=>{let F=[];return ce.forEach((ue,me)=>{ge[me]&&F.push(ue.id)}),F.length||(F=pe),F[Math.floor(Math.random()*F.length)]},[t,e.exrand_difficulty,[0,1,2,3,4,5,6,7]],[n,e.exrand_source,["P"]]),w=await A(`/problem/list?difficulty=${m[0]}&type=${m[1]}&page=1`),b=w.currentData.problems.count,v=Math.ceil(b/50),E=Math.floor(Math.random()*v)+1;w=await A(`/problem/list?difficulty=${m[0]}&type=${m[1]}&page=${E}`);let z=w.currentData.problems.result,j=Math.floor(Math.random()*z.length),P=z[j].pid;location.href=`/problem/${P}`};g.on("click",f)},`
.exlg-rand-settings {
    position: relative;
    display: inline-block;
...
`);c.reg_hook_new("rand-training-problem","题单内随机跳题","@/training/[0-9]+(#.*)?",{mode:{ty:"enum",vals:["unac only","unac and new","new only"],dft:"unac and new",info:["Preferences about problem choosing","随机跳题的题目种类"]}},({msto:e,args:t})=>{let n=e.mode.startsWith("unac")+e.mode.endsWith("only")*-1+2;!t.length||o(t[0].firstChild).clone(!0).appendTo(t).text("随机跳题").addClass("exlg-rand-training-problem-btn").on("click",()=>{let r=h.training,i=[];if(r.problems.some(a=>{r.userScore.score[a.problem.pid]===null?n&1&&i.push(a.problem.pid):r.userScore.score[a.problem.pid]<a.problem.fullScore&&n&2&&i.push(a.problem.pid)}),r.problemCount){if(!i.length)return n===1?k("您已经做完所有新题啦！"):n===2?k("您已经订完所有错题啦！"):k("您已经切完所有题啦！")}else return k("题单不能为空");let l=~~(Math.random()*1e6)%i.length;location.href="https://www.luogu.com.cn/problem/"+i[l]})},e=>{let t=o(e.target).find("div.operation");return{result:t.length>0,args:t}},()=>o("div.operation"),`
...
`);var le="VER 5.6.1";c.reg_chore("update","检查更新","1D",".*",null,()=>{J((e,t)=>t==="<<"&&k(`<p>检测到新版本 ${e}，点击确定将安装。</p>`,"检测到新版本",()=>location.href=`https://hub.fastgit.xyz/extend-luogu/extend-luogu/raw/${e}/dist/extend-luogu.min.user.js`))});var fe=["$","#<@"].map(e=>e.split("")),ie={Ty:{"-":"添加",x:"删减","!":"重大","*":"修改","^":"修复",$:"重构"},Op:{"-":"特性","?":"文档","#":"依赖","<":"代码风格",">":"命令",M:"模块",H:"钩子","@":"GitHub Action"}};c.reg("update-log","更新日志显示","@/.*",{last_version:{ty:"string",priv:!0},style:{ty:"enum",vals:["Commit Message","自然语言"],get:"id",info:["The way to display log","显示 Log 的方式"]},keep_dev:{ty:"boolean",dft:!0,info:["Keep developer messages","保留开发者更新信息"]}},({msto:e})=>{if(location.href.includes("blog"))return;let t=GM_info.script.version,n=!1,r=i=>{let l=`<div class="exlg-update-log-text" style="font-family: ${C["code-block-ex"].copy_code_font};">`;return i.split(`
`).forEach(a=>{let s=a.trimStart();if(s.length){if(e.keep_dev)n=!1;else if(s[0]===":"){if(!n)return}else if(fe.some((d,g)=>d.includes(s[g]))){n=!1;return}else n=!0;e.style===1&&s[0]!==":"&&(a=" ".repeat(a.length-s.length)+ie.Ty[s[0]]+ie.Op[s[1]]+s.substring(2))}l+=`<div>${a.replace(/ /g,"&nbsp;")}</div>`}),l+"</div>"};switch(q(e.last_version,t)){case"==":break;case"<<":k(r(le),`extend-luogu ver. ${t} 更新日志`);case">>":e.last_version=t}},`
...
`);c.reg("virtual-participation","创建重现赛","@/contest/[0-9]*(#.*)?",{},()=>{if(h.contest.name.match("Virtual Participation")){let e=o(".items"),t=()=>{(location.hash||"#main")==="#problems"&&o(".pid").length!==0&&h.contest.startTime>te(1e3)&&(o("a.title.color-default").on("click",()=>{k("比赛尚未开始, 请开始后再查看题目")}),o("a.title.color-default").removeAttr("href"))};e.on("click",t),t();return}if(h.contest.endTime>te(1e3)){R("Contest has not started or ended.");return}o("<button id='exlg-vp' class='lfe-form-sz-middle'>重现比赛</button>").appendTo(o("div.operation")).click(async()=>{k(`<div>
                ...
                <p>开始时间：<input type="date" id="vpTmDt"/> <input type="time" id="vpTmClk"/></p>
                    ...
                    "hostID":${N.uid}
                }`;l=await U("/fe/api/contest/new",a);let s=l.id.toString();switch(l.status??200){case 200:s=l.id.toString();break;default:M(`Failed to modify contest ${s} with status code ${l.status}.`)}l=null;try{l=await U(`/fe/api/contest/editProblem/${s}`,`{
                            "pids":[${r}],
                            "scores":{${i}}
                        }`)}catch{k("<p>本场比赛的题目不公开</p>","重现赛创建失败");return}let d=await A(`/contest/edit/${s}`);await U(`/fe/api/contest/join/${s}`,`{"code": "${d.currentData.contest.joinCode}"}`),k(`<p>邀请码: ${d.currentData.contest.joinCode}</p>
