...
mod.reg("keyboard-and-cli", "键盘操作与命令行", "@/.*", {
    ...
    const cli_log = (sp, ...tp) => {
        cli_is_log = true;
        const m = sp.map((s, i) => s.split(/\b/).map((w) => cli_lang_dict[w]?.[cli_lang - 1] ?? w).join("") +
            (tp[i] || "")).join("");
    ...
    const cmds = [
        gcmd("help", "[cmd: string]", [
            ...
            if (!cmd) cli_log`exlg cli. current language: ${cli_lang}, available commands: ${Object.keys(cmds).join(", ")}`;
            else {
                const f = cmds[cmd];
        ...
        gcmd("un", "name: string", [
            ...
            $.get(`/api/user/search?keyword=${name}`, (res) => {
                if (!res.users[0]) cli_error`un: unknown user "${name}".`;
        ...
        gcmd("cp", "pid: string", [
            ...
            "跳转至题目编号为 <pid> 的题目界面。",
        ], (pid) => location.href = `/problem/` + `${pid}`.toUpperCase()),
    ].reduce((tot, cmd) => {
        tot[cmd.name] = cmd;
        return tot;
    }, {});
