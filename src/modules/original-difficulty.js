...
mod.reg_pre("original-difficulty", "显示原始难度", ["@/problem/CF.*", "@/problem/AT.*"], {
    ...
    if (location.pathname.includes("CF")) {
        ...
        const dif = JSON.parse(sto["^atdiff-fetch"].atdiff);
        const pid = lg_dat.problem.description.match(/^.{22}[-./A-Za-z0-9_]*/)[0].match(/[^/]*$/);
        if (!(pid in dif)) resolve(undefined);
        else resolve(Math.round(dif[pid] >= 400 ? dif[pid] : 400 / Math.exp(1.0 - dif[pid] / 400)));
    }
}), ({ pred }) => {
    const x = document.querySelectorAll("div.field"),
        y = x[3].cloneNode(true);
    x[3].after(y);
    const t = y.querySelectorAll("span");
    t[0].innerText = "原始难度";
    t[1].innerText = "获取中";
    pred.then((d) => {
        if (d === undefined) d = "不可用";
        t[1].innerText = d;
    });
}, `
`, "module");
