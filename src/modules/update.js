...
mod.reg("update-log", "更新日志显示", "@/.*", {
    ...
    style: {
        ty: "enum", vals: ["Commit Message", "自然语言"], get: "id", info: [
            "The way to display log", "显示 Log 的方式",
        ],
    },
    keep_dev: {
        ty: "boolean", dft: true, info: [
    ...
    const fix_html = (str) => {
        ...
        str.split("\n").forEach((e) => {
            ...
            if (trmde.length) {
                ...
                if (msto.style === 1 && trmde[0] !== ":") e = " ".repeat(e.length - trmde.length) + human_lang.Ty[trmde[0]] + human_lang.Op[trmde[1]] + trmde.substring(2);
            }
