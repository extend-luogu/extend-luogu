...
mod.reg("messages", "新消息提醒", [ "@/.*" ], {},  () => {
    ...
    function msg_show(msg) {
        ...
        $(`#exlg-message-close-${ msg["id"] }`).click(function () { $(`#exlg-message-${ msg["id"] }`).remove() })
        $(`#exlg-message-show-${ msg["id"] }`).click(function () { $(`#exlg-message-${ msg["id"] }`).remove() })
        ...
        function() {
            $.ajax({
                ...
                success: function(data){
                    let chat = data["currentData"]["latestMessages"]["result"]
                    if (ever_fetched && chat[0]["id"] !== chat_last_fetch[0]["id"]) {
                        let new_msg = []
                        for (let i = 0; i < data["currentData"]["latestMessages"]["count"]; ++i) {
                            if (chat[i]["id"] > last_fetch_id) if (chat[i]["sender"]["uid"] !== uid_self) new_msg.push(chat[i])
                            else break
                        }
