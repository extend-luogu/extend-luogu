...
mod.reg_hook_new("user-comment", "用户备注", ".*", {
    ...
    if (!result) { // Note: 刚刚加载时
        ...
        if (/^\/user\/[1-9][0-9]{0,}$/.test(location.pathname)) {
            ...
            $cbtn.on("click", () => {
                exlg_alert(`<div>请设置用户 <span style="font-family: Consolas, monospace;">${orin} (uid: ${uid})</span> 的备注名。<br/><small>留空则清除备注。</small></div><input exlg-badge-register type="text" style="font-family: Consolas, monospace;line-height: 1.5;padding: .1em;" class="am-form-field exlg-badge-input" placeholder="${orin}" name="username" id="exlg-user-com-input">`, "exlg 用户备注", () => {
