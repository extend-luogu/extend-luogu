...
mod.reg_hook_new("user-comment", "用户备注", ".*", {
    ...
    const _setcomment = ($nm, com) => {
        ...
        if (msto.direct_display) $nm.text(com);
        else $nm.append(`<span class="exlg-usercmt exlg-usercom-tag">(${com})</span>`);
    };
    if (!result) {
    ...
    args.forEach((arg) => {
        const $arg = $(arg),
            uid = arg.href.split("/").lastElem();
        if ($arg.hasClass("exlg-usercmt")) return;
